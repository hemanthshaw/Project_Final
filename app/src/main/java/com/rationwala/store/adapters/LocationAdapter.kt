...
import com.rationwala.store.AddAddress
import com.rationwala.store.MyAddress
...
class LocationAdapter(var activity: MyAddress, var lis:MutableList<LocationClass>): RecyclerView.Adapter<LocationAdapter.Myholder>() {
    ...
    override fun onBindViewHolder(p0: Myholder, p1: Int) {
        ...
        p0.edit!!.setOnClickListener {
            activity.startActivity(Intent(activity,AddAddress::class.java)
                .putExtra("lname",lis[p1].locationname).putExtra("hno",lis[p1].address)
                .putExtra("landmark",lis[p1].landmark).putExtra("area",lis[p1].area)
                .putExtra("pincode",lis[p1].pincode).putExtra("from",lis[p1].key)
                .putExtra("lati",lis[p1].lati).putExtra("longi",lis[p1].longi)
                .putExtra("mobile",lis[p1].mobile))
        }

        p0.check!!.setOnClickListener{
            activity.addAddress(lis[p1])
        }
    ...
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): LocationAdapter.Myholder {
        ...
        v.setOnClickListener {
            ...
            activity.addAddress(lis[myholder.adapterPosition])
