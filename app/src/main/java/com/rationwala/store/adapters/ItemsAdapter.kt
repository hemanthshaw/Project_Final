...
class ItemsAdapter(
    ...
    override fun onBindViewHolder(p0: Myholder, p1: Int) {
        ...
        if(lis[p1].cost[ list[p1]].quantity.length<=6)
        p0.quan!!.text = lis[p1].cost[ list[p1]].quantity
        else
            p0.quan!!.text = lis[p1].cost[ list[p1]].quantity.substring(0,6)+".."
        ...
        Picasso.with(activity).load(lis[p1].image).placeholder(R.mipmap.ic_launcher).into(p0.im!!)
        var x= 0
