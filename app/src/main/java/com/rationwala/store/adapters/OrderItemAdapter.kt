package com.rationwala.dukana.adapters
...
class OrderItemAdapter(var activity: Context, var lis:MutableList<CartClass>): RecyclerView.Adapter<OrderItemAdapter.Myholder>() {
    override fun onBindViewHolder(p0: Myholder, p1: Int) {
        ...
        p0.mrp!!.text ="₹ "+ String.format("%.2f",lis[p1].item.cost.toFloat())
        p0.mrp!!.setPaintFlags(p0.mrp!!.getPaintFlags() or Paint.STRIKE_THRU_TEXT_FLAG)
        p0.discount!!.text = "You save ₹"+ String.format("%.2f",lis[p1].item.discount.toFloat())
        p0.cost!!.text = "₹"+ String.format("%.2f",(lis[p1].item.cost.toFloat()-lis[p1].item.discount.toFloat()))
