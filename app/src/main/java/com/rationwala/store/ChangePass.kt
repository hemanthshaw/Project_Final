...
class ChangePass: AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        change.setOnClickListener {
            ...
            {
                Toast.makeText(this@ChangePass,"Please connect to internet",Toast.LENGTH_LONG).show()

            }
            else
            {
                var pd = ProgressDialog(this@ChangePass)
                ...
                user.reauthenticate(credential).addOnCompleteListener(object : OnCompleteListener<Void> {
                    override fun onComplete(p0: Task<Void>) {
                            ...
                            user.updatePassword(npass.text.toString()).addOnCompleteListener(
                                object : OnCompleteListener<Void> {
                                    override fun onComplete(p1: Task<Void>) {
                                        if (!p1.isSuccessful) {
                                            pd.dismiss()
                                            Toast.makeText(this@ChangePass,"something went wrong..try later",Toast.LENGTH_LONG).show()

                                        }
                                        else
                                        {
                                            pd.dismiss()
                                            Toast.makeText(this@ChangePass,"successfully updated",Toast.LENGTH_LONG).show()
                                            finish()
                        ...
                        {
                            pd.dismiss()
                            Toast.makeText(this@ChangePass,"wrong current password",Toast.LENGTH_LONG).show()
                        }
                    }
                })
